...
def index(request):
    ...
    if user.is_authenticated():
        ...
        if timezone.now() > profile.expire_date:
            profile.state = False
            profile.save()
    else:
        profile = None
...
def signin(request):
    ...
    if request.method == 'GET':
        return render(request, 'signin.html', {'next': request.GET.get('next', None)})
        ...
        if user is not None:
            ...
            #Redirect to another signin page and show error. (Can be done in a better way through js)
                ...
                return render(request, 'signin.html', {'next': request.POST.get('next', '/'), 'error': 'User is not active',})
        else:
            return render(request, 'signin.html', {'next': request.POST.get('next', '/'), 'error': 'Incorrect login information',})
...
def charts(request):
    ...
    return render(request, 'charts.html', {'user': user})
